...
Estruturação automatizada dos **alvos de conservação** presentes no Plano de Redução de Impactos de Infraestruturas Viárias Terrestres sobre a Biodiversidade - PRIM-IVT.

Este repositório contém um pipeline em R desenvolvido para extrair, padronizar e organizar os alvos de conservação (Fauna, Flora, Fitofisionomias e Patrimônio Espeleológico) a partir dos arquivos brutos disponíveis no Material Suplementar do PRIM IVT, transformando o conteúdo textual em uma base de dados estruturada.
...
O script principal percorre automaticamente todos os arquivos *.csv separados por biomas na pasta `data_raw`.
